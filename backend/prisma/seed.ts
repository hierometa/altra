...
const cleanDB = async () => {
  // Clear/clean out the database
  await prisma.invoice.deleteMany();
  await prisma.user.deleteMany();
  // Reset the sequence/primary key to keep the ids consistent
...
async function main() {
  ...
  const createdUsers = await Promise.all(
    hashedUsers.map((user) => prisma.user.create({ data: user })),
  );
  ...
  const invoices = [
    ...
    ...Array.from({ length: 75 }, (_, index) =>
      generateRandomInvoice(index % createdUsers.length, createdUsers),
    ),
  ];
  // Seed Invoices data
  for (const invoice of invoices) {
    // Use forloop to keep same id/index if seed re-ran
    await prisma.invoice.create({ data: invoice });
  }

  console.log(
    `DB successfully seeded! ${users.length} users and ${invoices.length} invoices in DB!`,
  );
}
...
const generateRandomInvoice = (index: number, createdUsers: any[]) => {
  const randomVendor =
    randomVendors[Math.floor(Math.random() * randomVendors.length)];
  const randomCity =
  ...
  const randomAmount = (Math.random() * 200).toFixed(2); // Random amount between 0 and 200

  // Generate random date within December 2024
  const randomDay = Math.floor(Math.random() * 31) + 1; // Random day from 1 to 31
  const randomDate = new Date(`2024-12-${randomDay}`);
  // const randomSt = Math.floor(Math.random() * 60) + 6; // Random day from 1 to 31
  const randomSt = () => {
    const lastDigits = [4, 5, 6, 7, 8, 9, 0]; // Allowed last digits
    const randomLastDigit = lastDigits[Math.floor(Math.random() * lastDigits.length)]; // Pick a random last digit
    const randomNumber = Math.floor(Math.random() * 10) * 10 + randomLastDigit; // Generate a random number in the form of XX where last digit is 6,7,8,9 or 0
    return randomNumber;
  };
  const randomStreetOrBoulevard = () => {
    const abbreviations = ['St.', 'Blvd.', 'Ave.', 'Cir.', 'Pl.', 'Rd.']; // Common abbreviations
  ...
  return {
    ...
    paid: Math.random() > 0.5, // Random boolean for paid or unpaid
